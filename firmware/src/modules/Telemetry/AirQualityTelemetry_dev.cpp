...
int32_t AirQualityTelemetryModule::runOnce()
{
    ...
    */
    ...
    // TODO: Revert to 30 sec interval once we can control PMS interval
    moduleConfig.telemetry.air_quality_interval = 3;    // interval in seconds
    ...
    if (firstTime) {
        ...
        if (moduleConfig.telemetry.air_quality_enabled) {     
            ...
            if (!aqi.begin_I2C(&Wire1)) {
                LOG_WARN("Could not establish i2c connection on peripheral bus");
                return disable();
            }
            // Look for the Sensor
        ...
        if (((lastSentToMesh == 0) ||
             !Throttle::isWithinTimespanMs(lastSentToMesh, 3000))){
